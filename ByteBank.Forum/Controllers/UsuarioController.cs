    ...
    public class UsuarioController : Controller
    {
        ...
        public async Task<ActionResult> EditarFuncoes(UsuarioEditarFuncoesViewModel modelo)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var funcoesSelecionadasPeloAdmin = modelo.Funcoes.Where(funcao => funcao.Selecionado).Select(funcao => funcao.Nome).ToArray();
                    var  resultadoAdicao = await UserManager.AddToRolesAsync(usuario.Id, funcoesSelecionadasPeloAdmin);
